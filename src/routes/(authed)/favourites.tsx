...
const filtersSchema = v.object({
  ...
  timeRange: v.optional(
    v.picklist(["long_term", "medium_term", "short_term"] as const),
  ),
});
...
export default function Favourites() {
  ...
  const currentKind = () =>
    type() === "Tracks" ? ("tracks" as const) : ("artists" as const);
  ...
  function setTimeRange(timeRange: TimeRange) {
    setSearchParams({
      timeRange: timeRange === "short_term" ? undefined : timeRange,
    });
  }
  ...
  onMount(() => {
    ...
    (["short_term", "medium_term", "long_term"] as TimeRange[]).forEach(
      (range, index) => {
        createShortcut(
          [String(index + 1)],
          (event) => {
    ...
    <main class="app-main p-8 md:p-12">
      ...
      <div class="flex flex-wrap items-center gap-0 mb-3">
        ...
        {(["Tracks", "Artists"] as ItemType[]).map((opt) => (
          ...
          >
            {opt} <span class="ml-2 text-[0.6rem] opacity-50">T</span>
          </button>
        ))}
