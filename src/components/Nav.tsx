...
export function Nav() {
  ...
  const logout = () => { clearStoredState(); navigate("/login"); };

  return (
    <Show when={mounted() && accessToken() && !hidden()}>
      <header class="flex items-center justify-between p-4 sm:p-5" style="border-bottom: 4px solid #0a0a0a">
        <span class="font-black text-xl tracking-tighter uppercase select-none">SPOTISTATS</span>
        <nav class="flex flex-wrap gap-0">
          {links.map(([href, label]) => (
            <A
              ...
              style={isActive(href) ? "background: #0a0a0a; color: #f0ede8" : "border: 4px solid #0a0a0a"}
            >
              {label}
            </A>
          ))}
